...
const testChromium = require('./chromium_test');
...
Actor.main(async () => {
    ...
    await testChromium({ headless: false });

    // Try to use playwright default
    await testChromium({ executablePath: undefined });
    await testChromium({ executablePath: process.env.APIFY_DEFAULT_BROWSER_PATH });
